...
class CONFIG:
    size = 32
    num_obj = 5
    model_detection_path = os.path.join(CURRENT_DIR, 'resource', 'models', 'model_detect.pt')
    model_classify_path = os.path.join(CURRENT_DIR, 'resource', 'models', 'model_classi.pth')
...
def auto_handle(cam_names, imgs):
    ...
    data_cams = STEP_1.handle(cam_names, imgs, CURRENT_DIR)
    # Phân loại lỗi
